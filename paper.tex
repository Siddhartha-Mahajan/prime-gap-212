...
\thanks{This manuscript is a substantially AI-generated mathematical research
artifact. Siddhartha Mahajan supplied the problem and source materials, gave
brief high-level directions, and arranged its release. He did not independently
derive or verify the mathematical argument. The research and manuscript were
generated in the ChatGPT web interface by GPT-5.6 Sol using Pro reasoning
effort; Codex was used afterward only for review and computational verification.
...
\subjclass[2020]{11N05, 11N36}
\keywords{Bounded prime gaps, Maynard--Tao sieve, distribution in arithmetic progressions, computer-assisted proof}

\begin{document}
\begin{abstract}
Building directly on Julia Stadlmann's 2026 breakthrough, which established
$H_1\le240$, we prove that every admissible $45$-tuple has infinitely many translates
containing at least two primes. In particular,
$\liminf_{n\to\infty}(p_{n+1}-p_n)\le 212$.
Stadlmann's equidistribution estimates for moduli with a large smooth factor
are the decisive distributional input; those estimates are used without
strengthening. A three-part partition lemma enlarges
...
$H_1\le246$. Julia Stadlmann \cite{Stadlmann26} obtained $H_1\le240$ by combining
...
Lemmas~3--7 of \cite{Stadlmann26}, which extend estimates from
...
hypotheses. We work with the prime indicator, so that no loss from a
prime minorant enters the variational inequality.
...
\begin{equation}\label{eq:prime-distribution}
 \sum_{q\in\Qset(x;\varepsilon_0)}|E_{\ind_\PP}(x;q,a)|
...
layer and $\rho=\ind_\PP$. The minorant parameters are $c_1=c_2=0$.
The roughness condition is automatic for the prime indicator, and its
mean is supplied by the prime number theorem. In the one-layer case the
...
Define
\begin{align}
 \alpha&=\gmin-2\delta-8\omega-61\tmax,\label{eq:alpha}\\
 S&=\frac12-2\delta-10\omega-68\tmax,\label{eq:S}\\
...
\begin{equation}\label{eq:capacities}
 a=\gamma-2\delta-8\omega_0-61\tau,\qquad
 c=\frac12-\gamma-2\omega_0-7\tau,\qquad
 d=8\omega_0+57\tau.
\end{equation}
These numbers are positive; in particular $d\ge49\tau$.
...
\begin{gather}
 ...
 c'\ge\frac{4c+d}{5}
      =\frac45\left(\frac12-\gamma\right)+\frac{29}{5}\tau.
...
\begin{align*}
 ...
 \ell_3&=-\gamma-8\omega_0-53\tau-\delta,&
 u_3&=-\gamma-8\omega_0-53\tau.
\end{align*}
...
The three closed intervals just constructed lie strictly inside the
intervals \eqref{eq:r-window}--\eqref{eq:d-window}. In each comparison
the lower-endpoint slack is $\zeta-\tau>0$ and the upper-endpoint slack
is $\tau>0$. This proves the proposition.
\end{proof}
...
\cite[Definitions~6--9]{Stadlmann26}. In particular,
$\HH(\xi_1,\xi_2,\xi_3)$ denotes the convolution family in Definition~9
of that paper. Its Type~I sequences have a smooth factor at scale
$x^\gamma$ with $\gamma\ge\xi_1-\eta$. Its Type~II sequences have two
Siegel--Walfisz factors, one at scale $x^\gamma$ with
...
for the prime indicator, by the Bombieri--Vinogradov theorem. We therefore
work on
\[
...
\cite[Lemmas~3--7]{Stadlmann26}. All inequalities used for this purpose
...
\subsection*{Type I}
Suppose first that $\xi_1-\eta\le\gamma\le1/2$. Set
\[
 \Delta_I(\gamma)=\gamma-4\omega-\frac13-\eta.
\]
Then
\[
 \Delta_I(\gamma)>\delta,\qquad
 3\gamma-12\omega-3\Delta_I(\gamma)=1+3\eta>1.
\]
To apply \cite[Lemma~5]{Stadlmann26}, we require a divisor with logarithm
in
\[
 (\gamma-3\tau-\Delta_I(\gamma),\ \gamma-3\tau).
\]
Put all large factors in this divisor and fill with smooth primes.
The initial logarithm is strictly less than the upper endpoint, because
\begin{equation}\label{eq:I-cap}
 2B<\xi_1-\eta-3\tmax.
\end{equation}
The total available logarithm exceeds the lower endpoint, since
\[
 \frac12-2\tau-\bigl(\gamma-3\tau-\Delta_I(\gamma)\bigr)
 =\frac16-4\omega-\eta+\tau>0.
\]
Lemma~\ref{lem:fill} supplies the required divisor.

For $1/2<\gamma\le1/2+2\omega+\tau$, use
\[
 \Delta_{I,h}=\frac{1-68\omega}{14}-\eta>\delta.
\]
Now $68\omega+14\Delta_{I,h}=1-14\eta<1$, and the required interval is
\[
 (1-\gamma-3\tau-\Delta_{I,h},\ 1-\gamma-3\tau).
\]
The same construction works: its upper endpoint exceeds $2B$, by
$2B<1/2-2\omega-4\tmax$, and the total logarithm exceeds its lower
endpoint. For $\gamma>1/2+2\omega+\tau$, the last case of
\cite[Lemma~5]{Stadlmann26} requires no divisor condition.
...
every convolution in $\HH(\xi_1,\xi_2,\xi_3)$. The constants and auxiliary
parameters can be chosen uniformly on the indicated ranges: the scalar
inequalities have positive margins, and the cutoff ranges have the
fixed tolerance $\eta$. The bound \eqref{eq:qmaximum} places every
modulus within $x^{1/2+2\omega}$.

The parameters satisfy
\[
 2\xi_1+3\xi_2<2,\quad \xi_2\le\xi_3,\quad
 \xi_1+9\xi_2<4,\quad 2\xi_1+\xi_2>1,\quad17\xi_2<7.
\]
Apply the decomposition of \cite[Proposition~2]{Stadlmann26} to
$\Qset^+$. Its proof applies to this restricted subfamily: the
convolution decomposition does not depend on the modulus, and its error
estimates are sums of absolute discrepancies, so restricting the outer
sum preserves every estimate. Since $\xi_2=2/5$, the two discarded sums
in that proposition are empty and $\rho=\ind_\PP$. Indeed, in the first
sum two exponents strictly greater than $1/5$ would have sum less than
$2/5$; in the second an exponent would have to lie strictly between
$1/5$ and $1/5$. There is consequently no mass loss and no negative
minorant term. This proves \eqref{eq:prime-distribution} on $\Qset^+$.
...
\[
 (0.0001580,\ 0.0000756,\ 0.0001434,\ 0.02725,\ 0.0001891,\ 0.0001879).
\]
...
\section{Research provenance, acknowledgment, and release status}
...
reasoning effort. Siddhartha Mahajan supplied the bounded-prime-gaps problem,
directed GPT-5.6 Sol to the relevant source materials, and gave brief,
...
only afterward to inspect the release package and rerun its computational
checks.

Mahajan has not independently derived or checked the proof and has not
verified it beyond asking Codex to review and test its own output. His role in
this release is prompting, provision of resources, curation, and publication.
No independent human mathematician has yet verified the analytic argument.
The successful binary64 and integer-interval computations are reproducibility
checks of the finite certificate; they are not independent human verification.
Readers should therefore treat the stated theorem as a candidate result
pending expert scrutiny.

The model is named as the author of this repository artifact to reflect how
the work was produced.

\subsection{Dependence on Stadlmann's work}
The argument depends decisively on Julia Stadlmann's distributional framework
and her proof of $H_1\le240$ \cite{Stadlmann26}. It does not strengthen her
equidistribution theorems. Its claimed new contribution is the partition of
the permitted support together with the finite sieve-function certificate
that uses those theorems to reach the smaller diameter.

\subsection{Lossfunk acknowledgment}
The problem resources, source collection, and research workspace used for
this project were obtained through Lossfunk, headed by Paras Chopra.
GPT-5.6 Sol and Siddhartha Mahajan thank Paras Chopra and the Lossfunk team
...
\url{https://github.com/Siddhartha-Mahajan/prime-gap-212}.
\end{center}
The repository does not currently contain a complete native export of the
Codex conversation.
