...
\]
Fix an integer $a=a(x)$ with $(a,P(x))=1$. Suppose, with arbitrary
logarithmic saving, that the discrepancy sum over $\mathcal R(x)$ is
bounded by $x(\log x)^{-C}$ for
\begin{enumerate}
\item every Type~II convolution in $\HH(\xi_1,2/5,2/5)$ arising from
the localized Heath--Brown decomposition below, and
\item every Type~III convolution in $\HH(\xi_1,2/5,2/5)$ arising from
that decomposition.
\end{enumerate}
For each requested saving, the implied constants may depend on the fixed
coefficient and derivative bounds of the localized factors, but are
uniform in their scale tuple.
...
at the endpoint $2/5$. We first record the uniformity in that argument.
Fix the saving $C>0$ required in the conclusion. Choose a fixed
localization parameter $C_0>2C+10$; it will be enlarged once below by an
amount depending only on $C$ and $K$. Put
\[
...
and use the finer-than-dyadic decomposition in
\cite[Equations~(3.8)--(3.13)]{PolymathDistribution}. More explicitly,
take
\[
 \Theta=1+(\log x)^{-C_0}
\]
and a smooth partition of unity on the multiplicative scales
$N\in\{\Theta^m:m\ge0\}$. The cutoff functions $\psi_N$ are supported
on $[\Theta^{-1}N,\Theta N]$ and, for every fixed $r\ge0$, satisfy
\begin{equation}\label{eq:HB-derivatives}
 \sup_t N^r|\psi_N^{(r)}(t)|
       \ll_r(\log x)^{rC_0}.
\end{equation}
The same bounds, with one additional fixed power of $\log x$, hold for
the factor carrying the logarithm in the Heath--Brown identity. The
implied constants are independent of the scale $N$.

For the term with $j$ truncated M\"obius factors, where $1\le j\le K$,
there are $2j\le20$ localized factors. The support restriction to
$[x,2x]$ leaves at most
\begin{equation}\label{eq:HB-number}
 O\bigl((\log x)^{2j(C_0+1)}\bigr)
 \le O\bigl((\log x)^{20(C_0+1)}\bigr)
\end{equation}
nonzero scale-tuples. The endpoint-collar function $e$ created when the
sharp cutoff is removed is supported on two intervals of combined
length $O(x(\log x)^{-C_0})$ and satisfies
$|e(n)|\ll\tau(n)^{O_K(1)}(\log x)^{O_K(1)}$, with exponents independent
of $C_0$. If $I$ denotes the union of these intervals and $H$ its total
length, then
\cite[Lemma~1.3(iii)]{PolymathDistribution} gives, uniformly in the
residue class,
\begin{equation}\label{eq:HB-collar-class}
 \sum_{\substack{n\in I\\n\equiv a\pmod q}}|e(n)|
 \ll \frac Hq\tau(q)^{O_K(1)}(\log x)^{O_K(1)}+x^{o(1)}.
\end{equation}
Summing \eqref{eq:HB-collar-class}, and treating the averaged term with
Lemma~1.3(ii), gives
\begin{equation}\label{eq:HB-collar}
 \sum_{q\in\mathcal R(x)}|E_e(x;q,a)|
 \ll x(\log x)^{-C_0+O_K(1)}
      +x^{1/2+2\omega+o(1)}.
\end{equation}
Here one uses
$\sum_{q\le Q}\tau(q)^{O_K(1)}/q\ll(\log Q)^{O_K(1)}$ and
$\sum_{q\le Q}\phi(q)^{-1}\ll\log Q$. This is the endpoint-collar
estimate used in \cite[Equations~(3.10)--(3.13)]{PolymathDistribution}.
Since $\omega<1/20$, the second term in \eqref{eq:HB-collar} has a
fixed power saving. Enlarge $C_0$, if necessary, so that
\eqref{eq:HB-collar}, the finitely many
Heath--Brown coefficients, and the initial triangle inequalities
contribute $O_C(x(\log x)^{-C})$.

We shall apply the two hypotheses of the lemma to each remaining
localized convolution with the saving
\begin{equation}\label{eq:HB-saving-budget}
 C'=C+20(C_0+1)+2.
\end{equation}
Such a saving is available because the hypotheses hold with arbitrary
logarithmic saving. Summing
\eqref{eq:HB-number} terms then leaves the required saving $C$.

We also verify the common bounds required in the statement. Every localized factor is one of
$\psi_N\mu_{\le}$, $\psi_N$, or its logarithmic variant. Equations
\eqref{eq:HB-derivatives} and the divisor bound give a coefficient-sequence
bound $(\log x)^{O_{C_0,K}(1)}$, with constants common to every scale
$N$. With these constants fixed, \cite[Lemma~3.4]{PolymathDistribution}
shows that a factor at any scale at least $x^c$, for fixed $c>0$, has
the Siegel--Walfisz property with constants independent of its scale.
The proof of that lemma also shows the required closure:
if $\alpha$ and $\beta$ have uniform coefficient-sequence bounds,
$\beta$ is Siegel--Walfisz, and its scale is at least $x^c$, then
$\alpha\star\beta$ is Siegel--Walfisz, with a bound depending only on
$c,C_0,K$ and the requested saving. A group of at most twenty factors
whose product scale is at least $x^c$ contains a factor at scale at
least $x^{c/20}$. That factor is Siegel--Walfisz by Lemma~3.4(iii), and
iterating the closure statement over the other factors proves uniformly
that the whole grouped convolution is Siegel--Walfisz. Multiplication by the single factor
$\log N$ costs at most one power of $\log x$, which is covered by the
two-power reserve in \eqref{eq:HB-saving-budget}. Thus the hypotheses
are used only for this fixed, normalized class of localized sequences,
uniformly over the scale tuple.

It remains to locate the scales without losing an endpoint. For a
nonzero scale-tuple $(N_1,\ldots,N_{2j})$, put
\[
 X_N=\prod_iN_i,\qquad
 t_i=\frac{\log N_i}{\log X_N}.
\]
Then $t_i\ge0$ and $\sum_i t_i=1$ exactly. The support relations for
the partition give, for every $S\subseteq\{1,\ldots,2j\}$,
\begin{equation}\label{eq:HB-scale-error}
 \begin{aligned}
 X_N&=x^{1+O_K(1/\log x)},\\
 \log_x\prod_{i\in S}N_i
   &=\left(\sum_{i\in S}t_i\right)\log_xX_N
     =\sum_{i\in S}t_i+O_K(1/\log x).
 \end{aligned}
\end{equation}
uniformly in the tuple. Since $\eta$ is fixed, take $x$ large enough
that the error in \eqref{eq:HB-scale-error} is smaller than $\eta/4$
for every subset $S$. Thus the decomposition reduces the von Mangoldt function on
$[x,2x]$, apart from the already bounded collar, to the uniformly
controlled convolutions
\[
 \alpha_1\star\cdots\star\alpha_s
\]
whose normalized exponents are the $t_i$ above. An individual factor
with $t_i\ge2\sigma$ is smooth by
\cite[Lemma~3.4(ii)]{PolymathDistribution}; equality causes no problem,
because \eqref{eq:HB-scale-error} and the common support enlargement by
$\Theta$ give a scale comparable to $x^{2\sigma}$, which is the
Vinogradov condition in that lemma.
...
of which has normalized exponent $\theta$ satisfying
\[
 \frac12-\sigma<\theta\le\frac12.
\]
In view of \eqref{eq:HB-scale-error}, the exponent of the smaller group
is greater than $1/2-\sigma-\eta/4$. Both group scales are bounded below
by a fixed positive power of $x$, so the uniform closure argument above
shows that both grouped convolutions are Siegel--Walfisz. Moreover,
\[
 \frac12-\sigma-\frac\eta4
   =\frac25-\frac\eta2>\frac25-\eta.
\]
The other endpoint is at most $1/2+\eta/4<3/5+\eta$.
...
$\xi_3=2/5$: the respective margins before the error in
\eqref{eq:HB-scale-error} are $3\eta/2$, $5\eta/4$, and $5\eta/4$.
Hence all remain positive after an error of at most $\eta/4$. The three
selected factors are smooth uniformly by the preceding paragraph. It is therefore covered
...
Since $(a,P(x))=1$ and every prime factor of $q$ is below $x$, one has
$(a,q)=1$. Terms with $(\ell,q)>1$ therefore vanish from both sides of
the discrepancy. Put
\begin{align*}
 S_\ell(a;q)&=
 \sum_{\substack{x/\ell\le m\le2x/\ell\\
                  m\equiv a\bar\ell\pmod q}}\alpha(m),&
 S_\ell^*(q)&=
 \sum_{\substack{x/\ell\le m\le2x/\ell\\(m,q)=1}}\alpha(m).
\end{align*}
Then
\[
 E_F(x;q,a)=\sum_{(\ell,q)=1}\beta(\ell)
 \left(S_\ell(a;q)-\frac{S_\ell^*(q)}{\phi(q)}\right).
\]
...
\end{proposition}
\begin{proof}
Fix $\varepsilon_0>0$, an integer $a=a(x)$ with $(a,P(x))=1$, and a
required logarithmic saving. Choose $\tau>0$ sufficiently small for the distribution estimates below, with
...
\section{Type-IIc window comparison}\label{app:typeIIc-window}
The statement of \cite[Lemma~6]{Stadlmann26} uses the divisor window
\begin{equation}\label{eq:window-52}
 (T-52\tau-\delta_*,\ T-52\tau),
 \qquad T=2\log_xr+2-\gamma-4\log_xq.
\end{equation}
One line near the beginning of its proof instead contains $100\tau$ in
place of $52\tau$; the subsequent divisor family and calculations use
$52\tau$. The construction in Proposition~\ref{prop:critical-factor}
does not depend on which of these two constants is intended. It produces
the interval
\begin{equation}\label{eq:window-101}
 [T-101\tau-\delta,\ T-101\tau].
\end{equation}
Since $\delta_*=\delta+\zeta$, $\zeta=10^{-6}$, and
$0<\tau\le10^{-8}$, the interval \eqref{eq:window-101} lies strictly
inside \eqref{eq:window-52}: its lower- and upper-endpoint slacks are
\[
 \zeta-49\tau\ge5.1\times10^{-7},
 \qquad 49\tau>0.
\]
It also lies strictly inside the window obtained from
\eqref{eq:window-52} by replacing $52\tau$ with $100\tau$; the
corresponding slacks are
\[
 \zeta-\tau\ge9.9\times10^{-7},
 \qquad \tau>0.
\]
Thus the Type-IIc application in this manuscript satisfies either
displayed version of the cited divisor window.
