...
of $C_0$. Lemma~1.3 of \cite{PolymathDistribution}, applied in each
progression and to the averaged term, gives
\begin{equation}\label{eq:HB-collar}
 \sum_{q\in\mathcal R(x)}|E_e(x;q,a)|
 \ll x(\log x)^{-C_0+O_K(1)}
      +x^{1/2+2\omega+o(1)}.
\end{equation}
Indeed, the bound in one residue class is
$((x(\log x)^{-C_0})/q+1)x^{o(1)}$; summing uses
$\sum_{q\le Q}q^{-1}\ll\log Q$ and
...
We also verify that the distribution estimates are uniform over these
scale-tuples. Every localized factor is one of
...
bound $(\log x)^{O_{C_0,K}(1)}$, uniformly in $N$. By
\cite[Lemma~3.4]{PolymathDistribution}, a factor at any scale at least
$x^c$, with fixed $c>0$, has the Siegel--Walfisz property uniformly in
the scale. The proof of that lemma also shows the required closure:
...
two-power reserve in \eqref{eq:HB-saving-budget}. These are precisely the uniformity
conventions required for the Type~II and Type~III hypotheses.
...
\begin{equation}\label{eq:HB-scale-error}
 X_N=x^{1+O_K(1/\log x)},
 \qquad
 \log_xN_i=t_i+O_K(1/\log x)
\end{equation}
uniformly in the tuple. Since $\eta$ is fixed, take $x$ large enough
that the error for every individual exponent and every sum of at most
three exponents is smaller than $\eta/4$. Thus the decomposition reduces the von Mangoldt function on
